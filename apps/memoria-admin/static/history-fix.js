...
(() => {
  ...
  function extractSingleUrl(text) {
    ...
    try { const u = new URL(value); return ['http:', 'https:'].includes(u.protocol) ? u.href : null; } catch (_) { return null; }
  }
  ...
  function asksLearnedSummary(text) {
    const value = normalized(text);
    return (value.includes('aprendeu') || value.includes('aprendendo') || value.includes('descobriu') || value.includes('descobertas')) && (value.includes('hoje') || value.includes('ultim') || value.includes('recent'));
  }
  async function persistEpisodeOnly(role, text) {
    const order = nextChatOrder();
    const timestamp = new Date().toISOString();
    const episodeId = `${chatSessionId}:${order}`;
    return api('/api/v1/episodes', {method:'POST',body:JSON.stringify({episode_id:episodeId,role,text,session_id:chatSessionId,order,timestamp,event_type:'chat_turn',topics:['chat','web','direct-memory']})});
  }
  async function persistBestEffort(role, text) {
    try { await persistEpisodeOnly(role, text); return true; }
    catch (error) { log('direct_chat_episode_error', {role, error:error.message}); return false; }
  }
  ...
  async function resolveServerKnowledge(text) {
    if (asksLearnedSummary(text)) {
      ...
      return {reply:`O servidor já consolidou ${recent.concepts || items.length} conceito(s) a partir de ${recent.observations || 0} observação(ões).\n\nMais recentes:\n${lines.join('\n')}`,hit:items[0]};
    }
  ...
  async function startSiteIngest(url) {
    const result = await api('/api/server/v1/site-ingest',{method:'POST',body:JSON.stringify({url})});
    ...
    await persistBestEffort('assistant',reply);
  ...
  async function directMemorySend() {
    ...
    if (!text) return;
    bubble(text,'user'); message.value='';
    const siteUrl=extractSingleUrl(text); const question=looksLikeQuestion(text)||asksLearnedSummary(text);
    const userPersisted = await persistBestEffort('user',text);
    if (!userPersisted) bubble('A mensagem não pôde ser gravada no BDR, mas a resolução continuará.','error','Persistência');
    try {
      if (siteUrl) await startSiteIngest(siteUrl);
      else if (!question) {
        const ingested=await api('/api/v1/conversation/ingest',{method:'POST',body:JSON.stringify({role:'user',text,session_id:profileSession,timestamp:new Date().toISOString()})});
        const relations=Array.isArray(ingested.relations)?ingested.relations:[];
        const reply=relations.length?`Registrado sem LLM: ${relations.length} relação(ões) extraída(s).`:'Registrado sem LLM. Nenhuma relação explícita foi extraída desta entrada.';
        bubble(reply,'assistant','Memoria.ia · direta'); await persistBestEffort('assistant',reply);
        showMetrics({mode:'direct-no-llm',operation:'ingest',relations:relations.length,input_tokens:0,output_tokens:0});
      } else {
        const resolved=await api('/api/v1/conversation/resolve',{method:'POST',body:JSON.stringify({query:text,session_id:profileSession})});
        const personalHit=String(resolved.status||'').toUpperCase()==='HIT'&&resolved.selected_context;
        if (personalHit) {
          const reply=String(resolved.selected_context); bubble(reply,'assistant','Memoria.ia · memória pessoal'); await persistBestEffort('assistant',reply);
          ...
          const learned=await resolveServerKnowledge(text).catch(error=>{log('server_knowledge_query_error',{error:error.message});return null;});
          if (learned) {
            bubble(learned.reply,'assistant','Memoria.ia · Server Knowledge'); await persistBestEffort('assistant',learned.reply);
            ...
            bubble(reply,'system','Memoria.ia · sem HIT'); await persistBestEffort('assistant',reply);
            showMetrics({mode:'direct-no-llm',operation:'resolve',status:'MISS',input_tokens:0,output_tokens:0});
          }
        }
      }
      // Do not reload BDR history here. If episode persistence is degraded, a reload
      // would erase the just-rendered answer even though resolution succeeded.
      const conversation=document.getElementById('conversation');
  ...
  send.onclick=async()=>{if(answerMode?.value==='direct'){await directMemorySend();return;} if(typeof originalSend==='function') return originalSend.call(send);};
  send.addEventListener('click',()=>{
    if(answerMode?.value==='direct')return; const text=message.value.trim(); if(!text)return;
    const signature=`${text}\u0000${Date.now()>>10}`; if(signature===lastPromotion)return; lastPromotion=signature;
    fetch('/api/v1/conversation/ingest',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({role:'user',text,session_id:profileSession,timestamp:new Date().toISOString()})}).then(async response=>{if(!response.ok){const body=await response.json().catch(()=>({}));throw new Error(body.detail||body.error||`${response.status} ${response.statusText}`);}}).catch(error=>console.warn('[memoria] profile promotion failed:',error.message));
  },true);
})();
