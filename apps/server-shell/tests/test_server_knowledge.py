...
def test_wikipedia_interface_terms_remain_addressed_but_low_weight(tmp_path):
    knowledge = ServerKnowledge(str(tmp_path / "knowledge"))
    row = evidence()
    row["terms"] = ["editar", "página", "bldc", "controle"]
    result = knowledge.learn_from_evidence(row)
    assert result["addressed"] == 4
