...
def test_gain_is_positive_after_reinforcement(tmp_path):
    k=ServerKnowledge(str(tmp_path/"knowledge")); k.learn_from_evidence(ev(["robotica","sensores"]))
    target=next(x for x in rank_epistemic_targets(k) if x["topic"]=="robotica")
    k.learn_from_evidence(ev(["robotica","controle"],"wikipedia","https://example.org/b"))
...
def test_gain_zero_without_new_evidence(tmp_path):
    k=ServerKnowledge(str(tmp_path/"knowledge")); k.learn_from_evidence(ev(["energia","bateria"]))
