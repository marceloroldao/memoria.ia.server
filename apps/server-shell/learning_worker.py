...
class LearningWorker:
    def __init__(self, knowledge, curiosity_data_dir: str, bdr=None, feedback=None, poll_seconds: float=2.0, max_evidence_per_cycle: int=5) -> None:
        ...
        self.poll_seconds=max(.5,poll_seconds); self.max_evidence_per_cycle=max(1,int(max_evidence_per_cycle)); self._stop=Event(); self._cycle_lock=Lock(); self._thread=Thread(target=self._loop,daemon=True,name="server-learning-worker"); self.offset=self._load_cursor()
