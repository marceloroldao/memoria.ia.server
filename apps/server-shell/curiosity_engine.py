...
class CuriosityEngine:
    def __init__(self, config) -> None:
        self.config=config
        self.data_dir=Path(config.curiosity_data_dir)
    ...
    def _save_state(self):
        self.data_dir.mkdir(parents=True, exist_ok=True)
        tmp=self.state_file.with_suffix(".tmp"); tmp.write_text(json.dumps(asdict(self.state),ensure_ascii=False,indent=2),encoding="utf-8"); os.replace(tmp,self.state_file)
    ...
    def _budget(self) -> bool:
        ...
        if self.state.hour_bucket != bucket:
            self.state.hour_bucket=bucket; self.state.requests_this_hour=0
    ...
    def _search(self, topic: str) -> list[tuple[str,str]]:
        endpoint=self.config.curiosity_search_url.replace("{query}",quote_plus(topic))
        html,_=self._get(endpoint, 750_000); parser=SearchHTML(); parser.feed(html)
        self.state.searches += 1
        return parser.links[:self.config.curiosity_results_per_search]

    def _read(self, url: str) -> dict[str,object]:
        html,_=self._get(url); parser=PageHTML(); parser.feed(html)
        text=parser.text[:self.config.curiosity_text_limit]
        terms=[]; counts={}
        for w in _words((parser.title+" "+parser.description+" "+text)):
    ...
    def _choose_topic(self) -> tuple[str,str]:
        ...
        if jump:
            candidates=[s for s in SEEDS if s not in trajectory] or SEEDS
            topic=self._rng.choice(candidates); self.state.jumps += int(bool(self.state.current_topic))
        ...
        options=[t for t in recent_terms if t not in self._known_terms]
        if options: return self._rng.choice(options), "novel_neighbor"
        return self.state.current_topic, "continue"

    def cycle_once(self):
        topic,reason=self._choose_topic(); self.state.current_topic=topic; self.state.reason=reason; self.state.status="searching"
        self.state.cycle += 1; self.state.last_cycle_at=_now(); self.state.trajectory=(self.state.trajectory+[topic])[-50:]
        self._event("topic",f"Pesquisando: {topic}",topic=topic,reason=reason,cycle=self.state.cycle)
        results=self._search(topic); discoveries=0
        for title,url in results:
            if url in self._seen_urls: continue
            self._seen_urls.add(url)
            try:
                page=self._read(url); terms=list(page["terms"])
                novelty=len([t for t in terms if t not in self._known_terms]) / max(1,len(terms))
                self._known_terms.update(terms)
                page.update({"kind":"evidence","message":str(page["title"] or title),"topic":topic,"novelty":round(novelty,3),"epistemic_status":"web_observation","confidence":0.25})
                self._event(**page); discoveries += int(novelty >= self.config.curiosity_novelty_threshold)
                if discoveries: break
            except Exception as exc:
                self.state.errors += 1; self._event("page_error",str(exc),url=url,topic=topic)
        if discoveries:
            self.state.discoveries += discoveries; self.state.stagnation=0
    ...
    def _loop(self):
        ...
        while not self._stop.is_set():
            if not self.state.enabled:
                self.state.status="paused"; self._save_state(); self._wake.wait(2); self._wake.clear(); continue
            try: self.cycle_once()
            except Exception as exc:
                self.state.errors += 1; self.state.last_error=str(exc); self.state.status="error"; self._event("error",str(exc),topic=self.state.current_topic); self._save_state()
    ...
    def dispatch(self, handler, path: str, query: dict[str,list[str]]) -> bool:
        ...
        try:
            if not suffix and handler.command == "GET":
                handler._write_json(200,self.snapshot()); return True
            if suffix in {"pause","resume","jump","cycle"} and handler.command == "POST":
                handler._write_json(200,self.action(suffix)); return True
        except ValueError as exc:
            handler._write_json(400,{"error":"invalid_curiosity_action","detail":str(exc)}); return True
