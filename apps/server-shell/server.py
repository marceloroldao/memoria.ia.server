...
class ShellHandler(BaseHTTPRequestHandler):
    ...
    def _health(self):
        ...
        self._write_json(200,{"schema":"memoria-server-health/v1","status":"online" if states=={"online"} else "degraded","shell":{"status":"online"},"curiosity":{"status":self.curiosity.state.status,"enabled":self.curiosity.state.enabled},"knowledge":{"concepts":k["concepts"],"observations":k["observations"],"storage":"bdr-canonical"},"components":{"memoria":m,"bdr_explorer":b,"model_gateway":g}})
    def _dispatch(self):
...
def main():
    ...
    ShellHandler.config=c;ShellHandler.auth=AuthManager(c.admin_username,c.admin_password,session_seconds=c.session_hours*3600);ShellHandler.autotests=AutonomousTestManager(c);ShellHandler.curiosity=CuriosityEngine(c);ShellHandler.site_ingest=SiteIngestManager(ShellHandler.curiosity,max_pages=200,max_depth=5);ShellHandler.knowledge=ServerKnowledge(str(Path(c.curiosity_data_dir).parent/"knowledge"));ShellHandler.growth=GrowthDiagnostics(c,ShellHandler.curiosity,ShellHandler.knowledge,write_lock=ShellHandler.episode_write_lock)
